...
protected:
        ...
        for (Iterator p = First(undefined_types); p.key; p = Next(p)) {
            ...
            if (!Getattr(defined_types, ty)) {
                ...
                if (!Getattr(defined_types, cp)) {
                    ...
                    Printv(f_go_wrappers, "func SwigClass", ty, "SetSwigcptr(ptr uintptr) *SwigClass", ty," {\n", NULL);
   ...
   * ---------------------------------------------------------------------- */
    int cgoGoWrapper(const cgoWrapperInfo *info) {
        ...
        String *parm_print = generateParamsString(info->parms, info->n, info->result, info->is_constructor, info->is_destructor, pi, p);
        ...
        if (info->base && info->receiver) {
            ...
            ++args;
            Printv(call, "C.uintptr_t(_swig_base.Swigcptr())", NULL);
        }
   ...
   * ------------------------------------------------------------------------ */
    ...
    typedef void(GO::*PROCESS_BASE_FUNC)(Node* base, int index);

    void visitBaseClasses(List *baselist, PROCESS_BASE_FUNC processFunc) {
        int i = 0;
    ...
    void visitAllBaseClasses(List *baselist, PROCESS_BASE_FUNC processFunc) {
        ...
        for (Iterator b = First(baselist); b.item; b = Next(b)) {
            ...
            visitAllBaseClasses(Getattr(b.item, "bases"), processFunc);
            i++;
        }
    }

    void visitAllBaseClassesOnce(List *baselist, PROCESS_BASE_FUNC processFunc, Hash* visited) {
        int i = 0;
        for (Iterator b = First(baselist); b.item; b = Next(b)) {
            if (!GetFlag(b.item, "feature:ignore")) {
                String* name = exportedName(Getattr(b.item, "sym:name"));
                if (Getattr(visited, name) == NULL) {
                    (this->*processFunc)(b.item, i);
                    Setattr(visited, name, NewString(""));
                }
                Delete(name);
            }
            visitAllBaseClassesOnce(Getattr(b.item, "bases"), processFunc, visited);
            i++;
        }
    }

    void visitAllBaseClassesOnce(List *baselist, PROCESS_BASE_FUNC processFunc) {
        Hash* visited = NewHash();
        visitAllBaseClassesOnce(baselist, processFunc, visited);
        Delete(visited);
    }
   ...
   * ------------------------------------------------------------ */
    ...
    void printBaseName(Node* base, int /*index*/) {
        String *go_base_class_name = goCPointerType(Getattr(base, "classtypeobj"), false);
        Printv(f_go_wrappers, "\t", go_base_class_name, "\n", NULL);
        Delete(go_base_class_name);
    }

    void generateSetSwigcptr(Node* base, int /*index*/) {
        String *go_base_name = exportedName(Getattr(base, "sym:name"));
        String *go_base_class_name = goCPointerType(Getattr(base, "classtypeobj"), false);
        Printv(f_go_wrappers, "\tp.SwigClass", go_base_name, " = *(p.swigGetSwigClass", go_base_name, "().(*", go_base_class_name, "))\n", NULL);
        Delete(go_base_name);
        Delete(go_base_class_name);
    }
    ...
    int classHandler(Node *n) {
        ...
        if (has_base_classes) {
            visitBaseClasses(baselist, &GO::printBaseName);
        }
        Printv(f_go_wrappers, "}\n\n", NULL);
        ...
        // A helper function for compatibility
        Printv(f_go_wrappers, "func Swigcptr", go_name, "(ptr uintptr) *", go_class_name," {\n", NULL);
        Printv(f_go_wrappers, "\treturn ", go_class_name, "SetSwigcptr(ptr)\n", NULL);
        Printv(f_go_wrappers, "}\n", NULL);

        Printv(f_go_wrappers, "func ", go_class_name, "SetSwigcptr(ptr uintptr) *", go_class_name," {\n", NULL);
        Printv(f_go_wrappers, "\tp := &", go_class_name, "{}\n", NULL);
        Printv(f_go_wrappers, "\tp.", go_swigcptr_name, " = ptr\n", NULL);
        if (has_base_classes) {
            visitBaseClasses(baselist, &GO::generateSetSwigcptr);
        }
        Printv(f_go_wrappers, "\treturn p\n}\n\n", NULL);
        ...
        if (is_director) {
            ...
            Printv(f_go_wrappers, "func (p ", go_class_name, ") DirectorInterface() interface{} {\n", NULL);
            Printv(f_go_wrappers, "\treturn nil\n", NULL);
            Printv(f_go_wrappers, "}\n\n", NULL);
        }

        Printv(f_go_wrappers, "type SwigIs", go_name, " interface {\n", NULL);
        ...
        if (has_base_classes) {
            ...
            for (Iterator b = First(baselist); b.item; b = Next(b)) {
                ...
                int r = addBase(n, b.item, bases, local, visited);
                if (r != SWIG_OK) {
                    return r;
                }
                Delete(bases);
            ...
            //ret = addExtraBaseInterfaces(n, parents, baselist);
            Delete(parents);
        }
...
CleanUp:
   ...
   * ------------------------------------------------------------ */
    ...
    int addBase(Node *n, Node *base, List *bases, Hash *local, Hash* visited) {
        ...
        for (Node *ni = Getattr(base, "firstChild"); ni; ni = nextSibling(ni)) {
            ...
            SwigType_push(ty, Getattr(node, "decl"));
            ...
            if (is_function) {
                ...
                if (generate == 0) {
                    ...
                    if (cn && Getattr(cn, "module") && Getattr(base, "module")) {
                        ...
                        if (Strcmp(moduleBase, moduleResult) != 0) {
                            // Result type is defined in current module, while
                            // the method itself is imported from other module
                            generate = 2;
                            Setattr(local, lname, NewString(""));
                        }
                    }
                ...
                if (Getattr(node, "sym:overloaded")) {
                    ...
                    String *receiver = class_receiver;
                    bool is_static = isStatic(node);
                    if (is_static) {
                        receiver = NULL;
                    }
                    String *go_name = buildGoName(Getattr(node, "sym:name"), is_static, false);
                    if (generate != 0) {
                        String* tmp = interfaces;
                        if (generate == 1) {
                            // Prevent generating interface
                            interfaces = NULL;
                        }
                        r = makeDispatchFunction(node, go_name, receiver, is_static, NULL, false);
                        interfaces = tmp;
                    } else {
                        r = makeDispatchInterface(node, go_name, receiver, is_static, NULL, false);
                    }
                    Delete(go_name);
                    if (r != SWIG_OK) {
                        return r;
        ...
        if (baselist && Len(baselist) > 0) {
            for (Iterator b = First(baselist); b.item; b = Next(b)) {
                ...
                int r = addBase(n, b.item, nb, local, visited);
                Delete(nb);
                if (r != SWIG_OK) {
                    return r;
                }
    ...
    String* generateParamsString(ParmList* parms, Node* n, SwigType* result, bool is_constructor, bool is_destructor, int pi, Parm* p) {
        String *parm_print = NewString("");
    ...
    int makeInterface(Node *n, String* /*name*/, String *go_name, String* /*overname*/, String* /*wname*/, List* /*base*/, ParmList *parms, SwigType *result, bool /*is_static*/) {
        if (!interfaces) {
            return SWIG_OK;
        }
        Wrapper* dummy = initGoTypemaps(parms);
        ...
        Parm* p = parms;
        String* parm_print = generateParamsString(parms, n, result, false, false, 0, p);
        Printv(interfaces, parm_print, "\n", NULL);
        DelWrapper(dummy);
   ...
   * ------------------------------------------------------------ */
    ...
    int goBaseMethod(Node *method_class, List *bases, Node *method, int generate) {
        ...
        if (generate != 0) {
            ...
            if (generate == 1)
                interfaces = NULL;
            makeWrappers(method, name, go_name, overname, wname, bases, Getattr(method, "parms"), result, is_static);
            interfaces = tmp;
        } else {
            bool add_to_interfaces = interfaces && !is_static && !overname && checkFunctionVisibility(method, NULL);
            if (add_to_interfaces) {
                makeInterface(method, name, go_name, overname, wname, bases, Getattr(method, "parms"), result, is_static);
            }
        }
   ...
   * ------------------------------------------------------------ */
    ...
    int goBaseVariable(Node *var_class, List *bases, Node *var, int generate) {
        if (isStatic(var)) {
            return SWIG_OK;
        }
        ...
        if (is_assignable(var)) {
            ...
            if (generate != 0) {
                ...
                if (generate == 1)
                    interfaces = NULL;
                r = makeWrappers(var, mname_set, go_name, NULL, wname, bases, parms, result, false);
                interfaces = tmp;
            } else {
                r = makeInterface(var, mname_set, go_name, NULL, wname, bases, parms, result, false);
            }
            if (r != SWIG_OK) {
                return r;
            }
        ...
        if (generate != 0) {
            ...
            if (generate == 1)
                interfaces = NULL;
            r = makeWrappers(var, mname_get, go_name, NULL, wname, bases, NULL, vt, false);
            interfaces = tmp;
        } else {
            r = makeInterface(var, mname_get, go_name, NULL, wname, bases, NULL, vt, false);
        }
        if (r != SWIG_OK) {
            return r;
        }
   ...
   * ------------------------------------------------------------ 
    ...
    void addConvertFunctions(Node *n, Hash *parents, List *bases, String* chain) {
        if (!bases || Len(bases) == 0) {
            return;
        }
        ...
        for (Iterator b = First(bases); b.item; b = Next(b)) {
            ...
            if (!GetFlag(b.item, "feature:ignore")) {
                String* newchain = Copy(chain);
                Append(newchain, "SwigClass");
                String *go_base_name = exportedName(Getattr(b.item, "sym:name"));
                Append(newchain, go_base_name);
                Append(newchain, ".");
                addConvertFunctions(n, parents, Getattr(b.item, "bases"), newchain);
                Delete(newchain);
                Delete(go_base_name);
            } else {
                addConvertFunctions(n, parents, Getattr(b.item, "bases"), chain);
            }
        }
    }

    int makeSwigConvertWrappers(Node *n, String *go_name, String *overname, String *wname, List *base, ParmList *parms, SwigType *result, bool is_static, String* toType) {

        // Copy and past from `makeCgoWrappers`
        cgoWrapperInfo info;
        ...
        String* argname = NewString("arg");
        String* ctype = gcCTypeForGoValue(n, Getattr(n, "type"), argname);
        Printv(f_c_wrappers, toType, "* ", wname, "(", ctype,") {\n", NULL);
        Printv(f_c_wrappers, "\treturn (", toType, "*)arg;\n", NULL);
        Printv(f_c_wrappers, "}\n\n", NULL);
    ...
    int addSwigConvertFunctions(Node *n, Hash *parents, List *bases) {
        ...
        for (Iterator b = First(bases); b.item; b = Next(b)) {
            ...
            if (Getattr(parents, go_base_name) == NULL) {
                ...
                String* tmp = interfaces;
                interfaces = NULL;
                int r = makeSwigConvertWrappers(n, go_name, NULL, wname, NULL, parm, result, false, Getattr(b.item, "classtype"));
                interfaces = tmp;
                convertFunction = false;
                if (r != SWIG_OK) {
                    Delete(go_name);
   ...
   * ------------------------------------------------------------ */
    ...
    int classDirectorConstructor(Node *n) {
        ...
        if (!is_ignored) {
            ...
            Printv(f_go_wrappers, "\tp := &", director_struct_name, "{*SwigClass", cn ,"SetSwigcptr(0), v}\n", NULL);
            ...
            if (cgo_flag) {
                Printv(call, "*(*", go_type_name, ")(", class_receiver, "SetSwigcptr(uintptr(C.", wname, "(C.int(swigDirectorAdd(p))", NULL);
            } else {
   ...
   * ------------------------------------------------------------ */
    ...
    void makeCgoDirectorDestructorWrapper(String *go_name, String *director_struct_name, String *director_sig) {
        ...
        Printv(f_go_wrappers, "\tswigDirectorLookup(c).(*", director_struct_name, ").", class_receiver, " = *", class_receiver, "SetSwigcptr(0)\n", NULL);
        Printv(f_go_wrappers, "\tswigDirectorDelete(c)\n", NULL);
        Printv(f_go_wrappers, "}\n\n", NULL);
    ...
    int oneClassDirectorMethod(Node *n, Node *parent, String *super) {
        ...
        if (!is_ignored) {
            ...
            if (GetFlag(n, "abstract")) {
                ...
                Printv(call, "swig_p.", go_type_name, ".Swigcptr()", NULL);
                if (cgo_flag) {
                    Printv(call, ")", NULL);
                }
            ...
            if (!GetFlag(n, "abstract")) {
                ...
                Printv(call, "p.(*", director_struct_name, ").", go_type_name, ".Swigcptr()", NULL);
                if (cgo_flag) {
                    Printv(call, ")", NULL);
                }
            ...
            if (SwigType_type(result) != T_VOID) {
                if (goTypeIsInterface(n, result)) {
                    result_wrapper = NewString("uintptr");
                } else {
   ...
   *--------------------------------------------------------------------*/
    ...
    int makeDispatchInterface(Node *n, String *go_name, String *receiver, bool is_static, SwigType *director_struct, bool is_upcall) {
    ...
    int makeDispatchFunction(Node *n, String *go_name, String *receiver, bool is_static, SwigType *director_struct, bool is_upcall) {
        ...
        for (int i = 0; i < nfunc; ++i) {
            ...
            if (receiver && use_receiver) {
                ...
                SwigType *tmp = goType(pi, Getattr(pi, "type"));
                String *nm = buildGoName(Getattr(ni, "sym:name"), false, isFriend(ni));
                Printv(start, "a[0].(SwigIs", tmp, ").SwigGet", tmp, "().(*", tm, ").", nm, NULL);
                Delete(nm);
                Delete(tmp);
                Delete(tm);
                advance_parm = true;
    ...
    String *goOverloadType(Node *n, SwigType *type) {
        ...
        if (Getattr(undefined_types, ty) && !Getattr(defined_types, go_type)) {
            ...
            if (Strcmp(wrapperType, "uintptr") != 0) {
                String* starWrapperType = NewString("*");
                Append(starWrapperType, wrapperType);
                Delete(wrapperType);
                return starWrapperType;
            } else {
                return wrapperType;
            }
        }
